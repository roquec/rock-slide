...
public class Movement : MonoBehaviour
{
	...
	void FixedUpdate ()
	{
		...
		GetComponent<Rigidbody2D>().velocity = new Vector2(runSpeed * h, GetComponent<Rigidbody2D>().velocity.y); 
		...
		if (prevousVerticalVel < -10f && GetComponent<Rigidbody2D>().velocity.y > -1f) {
			anim.SetTrigger ("Landing");
		}
		prevousVerticalVel = GetComponent<Rigidbody2D>().velocity.y;
